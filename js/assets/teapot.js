...
1.5,-0.84,0.075
	...
	// the data in geometry form. The model was distributed with these new
	...
	for ( var surf = minPatches ; surf < maxPatches ; surf ++ ) {
		...
		// so ignore it for geometry part of the loop if the lid is not desired
			...
			for ( i = 0 ; i < 3 ; i ++ ) {
				...
				for ( r = 0 ; r < 4 ; r ++ ) {
					...
					for ( c = 0 ; c < 4 ; c ++ ) {
						...
						// is the lid to be made larger, and is geometry a point on the lid
						// that is X or Y?
...
export function Teapot(scene) {
    ...
    const fitLid = true;
    const blinn = true;
    const geometry = TeapotBufferGeometry(size, segments, bottom, lib, body, fitLid, blinn);
